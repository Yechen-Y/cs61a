...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    n = player_score % 10 ##取玩家的个位数
    m = opponent_score // 10 % 10 ##取对手的十位数
    if abs(m - n) == 0:
        return 1
    else:
        return 3 * abs(m - n)
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided):
    """and the opponent has OPPONENT_SCORE points.
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    point_score = 0 ##玩家该局得分数
    if num_rolls == 0:
        point_score = boar_brawl(player_score, opponent_score)
    else:
        point_score = roll_dice(num_rolls, dice)
    return point_score
    # END PROBLEM 3
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    average_date = make_averaged(roll_dice, samples_count)(1, dice)## 以一个骰子为基准
    result = 1
    n = 2 ## form 2 to 10
    while n <= 10:
        tmp = make_averaged(roll_dice, samples_count)(n, dice)
        if average_date < tmp:
            average_date = tmp
            result = n
        n += 1
