    ...
    def nearest_bee(self):
        ...
        while pos.is_hive == False:
            if pos.bees == []:
                ...
                if dis >= self.lower_bound and dis <= self.upper_bound:
    ...
    def action(self, gamestate):
        if self.place.bees != []:
            if self.turns_to_chew == 0:
                ...
                eated_ant = random_bee(self.place.bees)
                Ant.reduce_health(eated_ant, eated_ant.health)
            else:
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):##是否可以占据同一位置
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
    ...
    def store_ant(self, ant):##将实例属性设置为ant
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        super().action(gamestate) ##原本的防御工作
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while next != None:
            if next.ant != None:
                Ant.double(next.ant)
                if next.ant.is_container and next.ant.ant_contained != None:
                    Ant.double(next.ant.ant_contained)
            next = next.exit
